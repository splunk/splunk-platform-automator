...
- **`update_mode: "merge"`** (default): Controls how existing apps are replaced during updates.
  ...
  - Applies to all deployment methods (Deployment Server, Cluster Manager, Deployer, Direct) and ITSI premium apps (search head, indexer, and license manager installs). For ITSI, clean mode removes each individual ITSI app directory before extracting the archive.
...
| `update_mode` | `"merge"` (default) or `"clean"`. Overrides the global `splunk_app_deployment.update_mode` for this ITSI app entry. In `merge` mode, the archive is extracted on top of the existing directories, preserving files not in the archive (e.g. `local/` customizations). In `clean` mode, each ITSI app directory is removed before the archive is extracted (full replace). |
