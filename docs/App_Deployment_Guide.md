...
- **`update_mode: "merge"`** (default): Controls how existing apps are replaced during updates.
  - **`merge`**: Overlays new files onto the existing app directory (rsync with `delete: no`). Files present on the target but not in the source are preserved. This keeps `local/` customizations across upgrades. This is the default for all app types.
  - **`clean`**: Removes the old app directory completely before installing the new version. This is a full replace — any `local/` customizations or files not in the source are deleted.
  - Applies to all deployment methods (Deployment Server, Cluster Manager, Deployer, Direct), ITSI premium apps (search head, indexer, and license manager installs), and ITSI content packs (file extraction only — the content pack API install and its `resolution` parameter are unaffected). For ITSI and content packs, clean mode removes each individual app directory before extracting the archive.
  - Can be overridden per-app with `update_mode` on individual app entries.
...
- name: "app_name"                    # Required: see "App name: folder-backed vs bundle-backed" below
  ...
  update_mode: clean|merge            # Optional: Update strategy (default: global setting or "merge")
...
| `update_mode` | `"merge"` (default) or `"clean"`. Overrides the global `splunk_app_deployment.update_mode` for this ITSI or content pack app entry. In `merge` mode, the archive is extracted on top of the existing directories, preserving files not in the archive (e.g. `local/` customizations). In `clean` mode, each app directory is removed before the archive is extracted (full replace). For content packs, this affects file extraction only; the content pack API install and its `resolution` parameter are unaffected. |
...
```yaml
splunk_app_deployment:
  update_mode: clean|merge               # Optional: Global update mode (default: "merge")
  apps:
    - name: string                       # Required: App folder name
      ...
      update_mode: clean|merge           # Optional: Per-app override (default: global setting)
                                         # "clean" = remove old app then install fresh
                                         # "merge" = overlay new files, keep existing extras
