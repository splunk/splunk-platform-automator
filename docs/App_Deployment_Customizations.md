...
- **Running playbooks standalone:** To run a `run_playbook` or `run_playbook_after_restart` task file manually (e.g. to re-run a configure playbook), use the wrapper `ansible/splunk_apps_playbook_run.yml`. See [App Deployment Apps Playbooks](App_Deployment_Apps_Playbooks.md).
