...
| Run a playbook | `spa run --list`, `spa run NAME --help`, then `spa run NAME --yes` when `requires_confirmation`. Playbooks with `requires_provisioned: true` also need hosts from `spa provision` |
...
| List / SSH / copy | `spa hosts list --status`, `spa hosts ssh NAME`, `spa hosts copy SRC DST` (`spa sh` / `spa shell` still SSH). `--status` skips live checks when unprovisioned; ssh/copy refuse until `spa provision` (same `check_provisioned` gate as `spa deploy`) |
...
`--hosts` on `deploy`, `run`, `suspend`, `resume`, and `hosts list` takes inventory names or roles (for example `indexer`). `spa validate` and `spa deploy` fail when Software/baseconfig dirs (and PS baseconfig apps) are missing, or when `source: local` apps are configured but `apps_dir` or the named app sources are missing. `spa deploy` also fails if any config host is missing from inventory unless the operator asked for `--allow-unprovisioned`. The same provision check applies to `suspend` / `resume`, `hosts ssh` / `copy`, and `spa run` when `# spa-run: requires_provisioned` is true. Discover playbooks with `spa --json run --list` then `spa run NAME --help`. If `requires_confirmation` is true, pass `--yes`. Skills should use full command names (`spa hosts ssh`, `spa validate`), not short aliases (`spa sh`, `spa val`). Do not remove a live host from `splunk_config.yml` and run `spa provision` — Terraform would terminate that instance until the provision destroy-guard exists (see ROADMAP).
