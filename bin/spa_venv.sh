...
# One implementation for the framework, labs, and the test runners.
# ==============================================================================
# Usage:
...
#   2. LAB/.venv when it exists (--lab DIR, or SPA_LAB_DIR)  -> pin per lab
#   3. SPA_HOME/.venv                                        -> shared default
#
# Ansible collections install next to the venv (<venv parent>/.collections)
# so a lab venv and the shared venv never fight over ~/.ansible.
# ==============================================================================
...
Usage: spa_venv.sh [options] [pip-package ...]
  ...
  --lab DIR            Lab dir; DIR/.venv wins over the shared venv
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --dir)
            ...
            ;;
        --lab)
            [[ $# -ge 2 ]] || { _spa_venv_fail "--lab needs a value"; break; }
            _spa_venv_lab="$2"
            shift 2
            ;;
        --python)
...
if [[ $_spa_venv_rc -eq 0 && "$_spa_venv_mode" != "help" ]]; then
    ...
    if [[ -z "$_spa_venv_dir" && -n "$_spa_venv_lab" && -d "${_spa_venv_lab}/.venv" ]]; then
        _spa_venv_dir="${_spa_venv_lab}/.venv"
    fi
    if [[ -z "$_spa_venv_dir" ]]; then
...
unset _spa_venv_self _spa_venv_bin_dir _spa_venv_home _spa_venv_lab \
