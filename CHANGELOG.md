...
- **Premium apps (ITSI)** – Splunk IT Service Intelligence as a premium pack (single archive, multiple apps, role-specific extraction):
  - **Config**: `premium_app: itsi` on the app entry; optional `itsi_version`, `app_sh_name` / `app_shc_name`, `itsi_notification_disable`. Source Splunkbase (app_id 1841) or local path.
  ...
  - **Removal**: Per-role removal (CM, LM, deployer, search head) with app list built from the archive; same `app_sh_name` / `app_shc_name` targeting for search heads. Fails if archive is not available or not listable.
