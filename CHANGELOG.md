...
- **Python 3.9+ is a hard requirement**: `spa_venv.sh --create` and `spa doctor` fail when `python3` is missing, older than 3.9, or lacks the `venv` module.
...
- **Docs** ([#61](https://github.com/splunk/splunk-platform-automator/issues/61)): replaced the 34-file documentation maze with eight lowercase, task-oriented pages plus a generated command catalog. [README.md](README.md) is the front door, [docs/user-guide.md](docs/user-guide.md) is the operator journey, and [AGENTS.md](AGENTS.md) is the spa-only agent contract. Consolidated app, AWS, install, secrets, upgrade, migration, and contributor guidance delegates flags and schemas to `spa --help`, `spa features`, `spa apps`, and `spa run`. Dropped the old known-issues / Ansible-version list (Ansible comes from `spa_venv`). Operator examples use `spa`, not raw Vagrant, Terraform, or `ansible-playbook`.

### Added

- **Missing Python requirements are a named error, not a traceback**: commands that read YAML or drive Ansible now fail with `spa cannot run this command: PyYAML, pydantic are not installed for …`, the state of every venv they checked (`not created` / `incomplete (no bin/activate)` / `cannot import yaml / pydantic` / `ok`), and the one command that fixes it — `spa venv … --create` when nothing exists, `--rebuild` when a venv is broken, `--reinstall` when the venv is fine but a requirement is absent, or the launcher path when spa simply started on the wrong interpreter. `spa venv`, `doctor`, `init`, and `environment` stay exempt because they are how an operator gets a venv. Lazily imported requirements (`boto3`, `lxml`, `jmespath`) get the same message instead of a `ModuleNotFoundError` from deep inside a subcommand, and the backend session gates lifecycle commands the same way so a future GUI cannot traceback. `spa --help` now leads with what SPA does rather than the `SPA_HOME` path contract.

- **`spa` runs on a venv that can actually run it**: a symlinked or PATH-resolved `bin/spa` re-execs into the resolved SPA venv (`SPA_VENV_DIR`, env `.venv`, then `SPA_HOME/.venv`) when the invoking `python3` cannot import `yaml` / `pydantic`, so only the installer wrapper needed that before. Venv resolution now also requires those imports, so an activatable but half-installed venv (interrupted pip, or another checkout still pinned through `SPA_VENV_DIR`) is skipped for `python3` / `ansible` instead of failing later with `ModuleNotFoundError`. `spa doctor` reports such a venv (`cannot import yaml / pydantic`) with a `spa venv … --reinstall --yes` fix, and `PyYAML` is now declared in `requirements.txt` because `spa` imports it directly rather than only through ansible-core.

- **`spa venv` lifecycle**: `spa venv --path` resolves the target without changing it; `--create`, `--reinstall`, `--upgrade`, and `--rebuild` manage the shared `SPA_HOME/.venv` (default) or `--environment` venv and require `--yes`. Explicit targeting prevents a stale exported `SPA_VENV_DIR` from redirecting repairs to another checkout. `spa doctor` now warns when that variable points outside the selected framework/environment or when Ansible/Pydantic are missing, with a matching `spa venv` repair command. The underlying `spa_venv.sh` also supports `--reinstall`, `--upgrade`, and `--rebuild`. `--upgrade` runs `pip install --upgrade -r requirements.txt` and `ansible-galaxy collection install --upgrade` so a newer Ansible (or collection) is picked up without deleting the venv; `--force` is not used on that collections path. The human confirmation names what actually changes — `will change the Python environment for this framework` (`will delete and recreate …` for `--rebuild`) instead of the playbook fallback `may change hosts (no playbook metadata)`, because no venv action touches a Splunk host. Create/reinstall/upgrade/rebuild stream step progress (`Creating virtual environment...`, `Installing packages...` / `Upgrading packages...`, `Installing Ansible collections...` / `Updating Ansible collections...` or `Ansible collections already installed`, since `.collections` lives beside the venv and survives a rebuild) and finish with `Virtual environment ready: PATH`; pip/galaxy logs stay hidden unless the command fails or `spa -v venv` is used.

- **Main `/spa` skill and environment registry** ([#54](https://github.com/splunk/splunk-platform-automator/issues/54)): `skills/spa/spa/` (`name: spa`). Software checklist in the [user guide](docs/user-guide.md#prepare-software) (employee baseconfig download: [https://go2.cisco.com/baseconfigs](https://go2.cisco.com/baseconfigs)). `spa environment` (alias `env`) `list` / `init` / `set` / `remove`; `spa init` aliases `environment init`. Default env parent `~/Splunk-Platform-Automator`; `spa environment set --env-dir` writes `paths.yml` `env_dir` only when non-default; `--software-dir` / `--baseconfig-dir` / `--apps-dir` write the same file for the shared installer, PS baseconfig, and local-apps trees. Init fills any still-unset shared path from its explicit options or discovered directories without replacing established globals. `spa environment set --default NAME` selects the registered fallback environment. One-off `init --env-dir PARENT --name NAME` (same as a dest path) does not persist the parent. `--name` is the registry key. `spa env init NAME --force` adopts an existing lab (default parent, `cwd/NAME`, or cwd) and adds it to the list. Default provider is aws (`providers.yml` only when set to virtualbox). `spa` resolves `--env`, cwd, or the registered default and applies its environment internally; no direnv or `.envrc` is needed. `spa env --export` remains for external tools. `spa environment remove` deletes the local env dir (`--yes`; `--force` if it looks deployed) and does not call `spa destroy`. A registered path that no longer exists is shown as `(missing)` by `spa environment list` and is ignored as the default env, so a stale entry never becomes `SPA_ENV_DIR`. Test runs isolate `XDG_CONFIG_HOME` per test (`tests/conftest.py`), so a suite run can never register an environment in the operator's `~/.config/spa/environments.yml`.
...
- **spa requires an env dir** (breaking for clone-equal `spa`): operators run `spa init`, select it with `spa --env NAME` / `spa environment set --default NAME`, or `cd` into it. `spa validate` (and other operator commands) from the clone/prefix exit 2 with an init hint when no valid default exists. Contributor `ansible-playbook` from a checkout is unchanged. `spa init` migrates clone `.vagrant/` into the env with config/inventory/tfstate.
...
- **Distribution M1 — path contract and env scaffold** ([#46](https://github.com/splunk/splunk-platform-automator/issues/46)):
  ...
  - Env selection is handled by installed `spa` through `--env`, cwd `.spa.yml` discovery, or the registered default; envs do not need `.envrc` or their own `bin/`. Pulled-back PS apps go to `$SPA_ENV_DIR/saved_base_config_apps/`. The host link page is `$SPA_ENV_DIR/config/index.html`.
...
- **`bin/spa_venv.sh` — shared Python virtualenv** for the framework, labs, and tests, so Ansible and Pydantic no longer have to come from Homebrew or the system Python:
  ...
  - `init_spa_dir.sh` creates the shared `$SPA_HOME/.venv` (or a lab `.venv` with `--venv`) when it is missing, then runs `spa_doctor.sh` (`--skip-doctor` to skip).
...
- **`bin/spa_doctor.sh` — host prerequisite check** (Python/venv, Terraform **only** when the lab uses `terraform.aws`, Vagrant **only** when the lab has a top-level `virtualbox:` section, Software/, and which `spa` on `PATH` wins). Interactive init runs that for you. Suggests `brew install …` on macOS.
