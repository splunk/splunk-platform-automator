...
Work on this branch is **3.0** (M1–M3 plus the follow-on issues below). Linux packages and Homebrew are later, not part of 3.0.
...
- **2.x `bin/` wrappers**: `init_spa_dir.sh`, `spa_env.sh`, `spa_doctor.sh`, `validate_splunk_config.sh`, `spash`, `splunk_config_aws.py`, and `splunk_config_licenses.py`. Operators use `spa`. `bin/spa_venv.sh` remains as the venv implementation (`spa venv`; contributors may still `source` it). `--lab` / `SPA_LAB_DIR` are gone (`--env` / `SPA_ENV_DIR`).

- **vagrant-aws / gecko-aws** from `Vagrantfile`. Cloud is `terraform.aws`. A leftover top-level `aws:` block is inventory-only.
...
- **Env inventory no longer symlinks `group_vars` into the repo**: framework vars stay in `$SPA_HOME/ansible/group_vars` and load through the `spa_group_vars` vars plugin. A leftover `$SPA_ENV_DIR/inventory/group_vars` symlink from a 2.x clone is removed by `spa init --force` / migrate, not at inventory parse time. The plugin loads those files as trusted templates (ansible-core 2.19+ leaves untrusted strings literal), so values such as `splunk_software` still resolve to a path.
...
- **Release bump regenerates `docs/commands.md`**: `scripts/bump-version.sh` runs `spa --no-agent agent schema --markdown` into the version commit (see [RELEASE.md](RELEASE.md)).
...
- **Env-dir run logs and compact progress** ([#71](https://github.com/splunk/splunk-platform-automator/issues/71)): mutating `spa` commands write redacted JSONL under `$SPA_ENV_DIR/logs/` (local timestamps with a numeric offset). Humans see one colored live line per **named group that actually runs** (imported playbook, role, app channel, ITSI/content-pack overlay, or upgrade step) with counters in place; `NO_COLOR` supported. There is no `[n/m]` index. `spa run` of first-party playbooks uses the same titles. The line ends with `ok` (green), `changed` (yellow), or `failed` (red), and counts task-host results as `changed N/M` plus `skipped N` next to the distinct `tasks N`. Agents keep a small stdout envelope plus `data.log` and sparse stderr heartbeats with task/host counts. `spa logs` / `--last` / `--follow` dump a run. `--ansible-output` on `spa logs` reconstructs PLAY/TASK/recap text and Terraform host diffs (`instance_type`, disk) from JSONL without storing a raw sidecar. Groups follow the provider: `terraform.aws` keeps the ordered Terraform steps, and `virtualbox` reads Vagrant's own output into one group per machine (`VM idx1`) with run-wide lines folded into the open group, box-download ticks dropped, and Vagrant errors (including its stderr headline on a non-zero exit) shown as `failed`. `--ansible-output` replays Vagrant text as Vagrant printed it. `spa suspend` / `resume` on VirtualBox log the same way instead of dumping raw Vagrant output. Ansible's own stderr notes (`[WARNING]: …`) are kept in the transcript and in `--ansible-output`, but never open a group of their own; a hard `ERROR!` is still printed and counted as failed.
...
- **Main `/spa` skill and environment registry** ([#54](https://github.com/splunk/splunk-platform-automator/issues/54)): `skills/spa/spa/` (`name: spa`). Software checklist in the [user guide](docs/user-guide.md#prepare-software) (employee baseconfig download: [https://go2.cisco.com/baseconfigs](https://go2.cisco.com/baseconfigs)). `spa environment` (alias `env`) `list` / `init` / `set` / `remove`; `spa init` aliases `environment init`. Default env parent `~/Splunk-Platform-Automator`; `spa environment set --env-dir` writes `paths.yml` `env_dir` only when non-default; `--software-dir` / `--baseconfig-dir` / `--apps-dir` write the same file for the shared installer, PS baseconfig, and local-apps trees. Init fills any still-unset shared path from its explicit options or discovered directories without replacing established globals. `spa environment set --default NAME` selects the registered fallback. One-off `init --env-dir PARENT --name NAME` (same as a dest path) does not persist the parent. `--name` is the registry key. `spa env init NAME --force` adopts an existing lab into the list. Default provider is aws (`providers.yml` only when set to virtualbox). `spa` resolves `--env`, cwd, or the registered default and applies its environment internally. `spa env --export` remains for external tools. `spa environment remove` deletes the local env dir (`--yes`; `--force` if it looks deployed) and does not call `spa destroy`. A registered path that no longer exists is shown as `(missing)` and is ignored as the default. Test runs isolate `XDG_CONFIG_HOME` so they never write the operator's `~/.config/spa/environments.yml`.

- **Generated `docs/commands.md`**: `spa --no-agent agent schema --markdown` renders `COMMAND_SCHEMA` as a checked-in CLI catalog (not a second user guide). `--json` / agent mode still print the JSON envelope. Drift-tested against the committed file.

- **`spa apps` Splunkbase search / snippet / download** ([#59](https://github.com/splunk/splunk-platform-automator/issues/59)): `spa apps search QUERY` returns compact hits (`app_id`, `name`, `title`, `type`, `kind`, `version`, short `summary`) for technology lookup. `--type` (`app`, `addon`) and `--kind` (`ta`, `premium_itsi`, `itsi_content_library`, `itsi_content_pack_single`, `es_not_premium`) filter the page. A query containing `itsi` pins app_id 1841 first; `es` pins 263. `spa apps snippet APP_ID` prints a kind-specific `apps[]` YAML block with a title comment; matching curated `ansible/apps_playbooks/` files are advertised (JSON `playbooks`, YAML comment) until `--customize` adds `customizations`. Search hits set `playbook: true` when a file matches kind plus name/`app_id`. Manual re-run: `spa run splunk_apps_playbook_run --help` / `--apps-playbook STEM` (or an env-relative path such as `ancustom/my_custom_playbook`; those files are not listed). `--source local` (short: `--local`) resolves the app folder or archive already in `apps_dir` (folder first) and also accepts a custom app folder name instead of an ID. `spa apps download APP_ID --yes` writes the archive to `apps_dir` only; `--extract` safely expands normal folder-backed apps and deletes the archive afterwards (kept when extraction fails). `--overwrite` with `--extract` replaces an existing app folder. Download credentials resolve from `splunk_app_deployment.splunkbase_username` / `splunkbase_password` in `splunk_config.yml` first (a `lookup('env', 'NAME')` value reads that variable; vault refs fall back), then `SPLUNKBASE_USERNAME` / `SPLUNKBASE_PASSWORD`. Download never prints YAML or edits `splunk_config.yml` (config apply is [#76](https://github.com/splunk/splunk-platform-automator/issues/76)). Search and Splunkbase-source snippet work without an env dir. ITE Work (5403) is not a separate SPA app (use 1841). Never prints Splunkbase passwords. Skill: `skills/spa/spa-apps/` (create-config loads it for the apps phase).

- **`install.sh --uninstall`** ([#72](https://github.com/splunk/splunk-platform-automator/issues/72)): removes the install prefix and the matching `spa` PATH wrapper (`--prefix` / `--bindir` same as install). Requires `--yes` (agents never prompt). Refuses `$HOME`, `/`, and non-SPA directories. Does not touch env dirs, Terraform state, AWS, `~/.config/spa`, or `~/.local/state/spa`. Not `spa destroy`.
...
- **VirtualBox spa provider** ([#56](https://github.com/splunk/splunk-platform-automator/issues/56)): `spa provision` / `destroy` / `suspend` / `resume` run Vagrant from `SPA_HOME` when `virtualbox:` is in config. `VAGRANT_CWD` is the framework; `VAGRANT_DOTFILE_PATH` is `$SPA_ENV_DIR/.vagrant`. Env dirs have no Vagrantfile. `spa hosts ssh` / `copy` use inventory. `spa doctor` compares installed VirtualBox against the drivers the installed Vagrant ships, requires `vagrant-vbguest`, warns for `virtualbox_WSL2` on WSL, and prints OS install commands when Vagrant or VirtualBox is missing. Provision refuses leftover `$SPA_ENV_DIR/.vagrant/machines/*/` state from another provider. The Vagrantfile needs `ansible` on `PATH`, so spa puts the venv's `bin` there and fails with the `spa venv` repair hint when Ansible is missing at all.
...
- **Distribution M3 — `install.sh` + framework tarball** ([#48](https://github.com/splunk/splunk-platform-automator/issues/48)):
  ...
  - Extract-anywhere: unpack the tarball, set `SPA_HOME`, run `spa venv --shared --create --yes`. See [docs/install.md](docs/install.md).
  ...
  - `spa validate` step 2 is `Controller directories (Software / baseconfig / apps / playbooks)` and prints one `OK` line per directory (`Software OK: …`, `Baseconfig OK: …`, `Local apps OK: …`, `Custom playbooks OK: …`), so a plain env reads `none configured` instead of a packed single line. The custom playbook line covers `$SPA_ENV_DIR/ancustom` (the default folder, no `.spa.yml` entry needed, reported `empty` while unused) plus every `playbook_dirs` entry, with the playbook count. A configured folder that is missing or outside the env dir fails the step instead of being skipped silently by `spa run --list`. Agent JSON carries `data.licenses` (the four config pairings the step asserts plus `files_found`) instead of the full per-file license scan; `--check-licenses` still adds `data.license_scan`, and `spa licenses --json` remains the full view. `data.controller_data` is only sent when the step fails, since the step message already names every directory. The agent envelope on stdout is compact JSON (no indentation) for every command. A plain `spa validate --agent` is ~0.8 KB instead of ~5.9 KB; human output is unchanged.

- **Distribution M2 — `spa` CLI** ([#47](https://github.com/splunk/splunk-platform-automator/issues/47)):
  - One user-facing entry: `bin/spa` (`lib/spa/`). Subcommands include `init`, `environment`, `venv`, `validate`, `doctor`, `features`, `apps`, `provision`, `deploy`, `suspend`, `resume`, `destroy`, `hosts`, `shell`, `aws`, `licenses`, `run`, `agent schema`.
  - Term is **env** (Splunk environment): `SPA_ENV_DIR` / `spa_env_dir`. `SPADirName: "{{ spa_env_dir | basename }}"`.
  - `spa init --force` never replaces `splunk_config.yml` unless `--example`. Old mixed clones print keep/remove and exit 2 until `--force`.
  - `spa init --venv` / `--python` / `--ansible VER` / `--pip PKG`; env `$DIR/requirements.txt` last. Runtime venv: `SPA_VENV_DIR` → `$SPA_ENV_DIR/.venv` → `$SPA_HOME/.venv` (usable venvs only).
  - `spa run`: `ansible/<stem>`, `verification/<stem>`, or `$SPA_ENV_DIR/<dir>/<stem>`. `--list` catalogs framework stems with `name — summary` (custom folders are `$SPA_ENV_DIR/ancustom` by default, plus any `playbook_dirs` in `.spa.yml`); `--json` keeps `source` plus metadata. `spa run NAME --help` describes a playbook without running Ansible. First-party playbooks use `# spa-run:` comments. Optional `requires_provisioned: true` runs the same host-up gate as `spa deploy` before Ansible. 3.0 renamed operator stems (`run_splunk_command` → `splunk_cli`, `provision_terraform_aws` → `aws_provision`, …); old stems still resolve with a hint. See [docs/migrate.md](docs/migrate.md).
  - Cup-style agent mode: auto-detect, versioned JSON envelope, `--agent` / `--no-agent`, `-y`. Skills and `.agent/workflows` call only `spa`.
  - `--yes` works after the subcommand. Confirmation is enforced in `LocalSpaSession`. Humans see a named prompt (default no). Agents/GUI never prompt. Lifecycle commands always require it; `spa run` uses catalog `risk` (fail-closed if metadata is missing). After a human confirms provision/destroy, Terraform gets `auto_approve=true`. Documented one-shot: `spa provision --yes && spa deploy --yes`.
  - `spa hosts` lists inventory (`list --status`), opens SSH (`ssh` / `spa sh` / `spa shell`), and copies files (`copy SRC DST`, remote side `HOST:PATH`, `-r` for directories). `--hosts` on `deploy`, `run`, `suspend`, `resume`, and `hosts list` takes names or roles. There is no `spa shell -l`; use `spa hosts list`. Human aliases: `val`, `prov`, `dep`, `des`, `sus`, `res`, `doc`, `lic`, `h`, `feat`. Skills keep full command names. Do not drop a live host from config and `spa provision` (Terraform would terminate it); ROADMAP tracks a provision destroy-guard and later `destroy --hosts` / `--all`.
  - `spa doctor` uses the same env resolution as other commands and checks only the provider tools that config needs (`terraform.aws` or `virtualbox:`, or `--aws` / `--virtualbox`). Missing host tools print an OS install command.
  - `spa licenses` parses sanitized license type, expiration, and add-on capabilities. `spa validate --check-licenses` rejects missing, invalid, expired, and wrong-entitlement files and warns within 30 days of expiration.
  - Infrastructure lifecycle uses a provider from `splunk_config.yml`. `terraform.aws` implements provision/destroy plus `spa suspend` / `spa resume`. `virtualbox` implements the same commands via Vagrant ([#56](https://github.com/splunk/splunk-platform-automator/issues/56)).
  - `spa hosts list --status` gets runtime host state through the configured provider. Ansible ping is skipped for instances the provider reports as not reachable, and skipped entirely when the env is not provisioned.
  - `spa.api` session protocol (`open_session`, `LocalSpaSession`, JSON-serializable `CommandResult`) sits behind the CLI. Skills and agents still invoke `bin/spa` only. `open_session(url=...)` is reserved. See [Contributing](docs/contributing.md#controller-architecture).
  - Native tool flags reach the wrapped tool: `spa aws --check-auth --json`, `spa licenses --json`, `spa sh idx1`.

- **Distribution M1 — path contract and env scaffold** ([#46](https://github.com/splunk/splunk-platform-automator/issues/46)):
  - `SPA_HOME` is the framework (`ansible/`, Terraform modules, `skills/`, `bin/`). `SPA_ENV_DIR` is the env (`config/splunk_config.yml`, optional `.spa.yml`, `inventory/hosts`, Terraform state, `.vagrant/`, `saved_base_config_apps/`). Unset both → the git clone remains valid for contributor `ansible-playbook`; operator `spa` requires an env (`spa init`, `--env`, cwd, or the registered default).
  - `spa init` scaffolds an env dir (example config, `.spa.yml`, `inventory/`, Terraform state dir). Never copies `ansible/`. `--example`; refuse overwrite unless `--force`. If the source already has `config/splunk_config.yml`, the existing env is **migrated** (config, inventory, Terraform state, `.vagrant/`) so a running deployment stays manageable from the new lab. `--from`, `--migrate`, `--keep-source`. An old clone copied as the target is converted in place (framework dirs stripped, state kept).
  - Inventory resolution sets `ANSIBLE_INVENTORY` to the env so a separate env does not pick up a clone's `config/splunk_config.yml`. Relative `../Software` prefers a sibling of the **env**, then a sibling of `SPA_HOME`. `.spa.yml` records `software_dir`, `baseconfig_dir`, and `apps_dir`. Config still overrides; env vars win over both.
  - Envs have no `bin/` of their own. Pulled-back PS apps go to `$SPA_ENV_DIR/saved_base_config_apps/`. The host link page is `$SPA_ENV_DIR/config/index.html`. When roots differ, Terraform modules stay under `$SPA_HOME/terraform/aws`; state stays under the env.
  - `requirements.yml` includes `ansible.windows` so `spa validate` can syntax-check `deploy_site.yml` (Windows UF `win_stat`).
