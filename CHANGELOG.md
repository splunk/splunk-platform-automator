...
- **VirtualBox spa provider** ([#56](https://github.com/splunk/splunk-platform-automator/issues/56)): `spa provision` / `destroy` / `suspend` / `resume` run Vagrant from `SPA_HOME` when `virtualbox:` is in config (clone layout only; env dirs stay Terraform). Vagrantfile no longer has vagrant-aws / gecko-aws. Top-level `aws:` remains inventory-only; use `terraform.aws`.
