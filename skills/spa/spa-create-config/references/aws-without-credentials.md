...
Use when `spa aws` cannot call AWS (no credentials, no boto3, or network blocked). The guided workflow **still works**; only live discovery and API validation are skipped.
...
spa aws --region <region> --validate \
