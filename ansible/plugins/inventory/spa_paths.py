#!/usr/bin/env python3
"""Resolve SPA_HOME, SPA_LAB_DIR, and related paths (Distribution M1).
...
SPA_LAB_DIR
    Lab root: config/splunk_config.yml, optional .spa.yml, inventory/hosts,
...
Resolution order
----------------
1. Environment: SPA_HOME, SPA_LAB_DIR, SPLUNK_CONFIG_FILE
...
def clone_root_from_this_file() -> Path:
    """Framework root when this file lives at ansible/plugins/inventory/spa_paths.py."""
    here = Path(__file__).resolve()
    # inventory → plugins → ansible → root
...
def resolve_shared_data_dir(
    ...
    first existing directory among lab-relative, SPA_HOME-relative, and
    ``$SPA_HOME/<home_leaf>``. If none exist, return the lab-relative path (the
    ...
    if yml_value:
        return _expand(str(yml_value), spa_lab_dir)

    rel = configured or default_rel
    candidates = [
        (spa_lab_dir / rel).resolve(),
...
class SpaPaths:
    ...
    def ansible_inventory(self) -> str:
        """ANSIBLE_INVENTORY value that prefers the lab over the clone config."""
    ...
    def export_env(self) -> Dict[str, str]:
        """Environment overlay for validate, spash, and ansible-playbook."""
        return {
            "SPA_HOME": str(self.spa_home),
            "SPA_LAB_DIR": str(self.spa_lab_dir),
...
def resolve_spa_paths(
    ...
    env_lab = (env.get("SPA_LAB_DIR") or "").strip()
    env_config = (env.get("SPLUNK_CONFIG_FILE") or "").strip()

    spa_yml_path = None
    spa_yml_data: Dict[str, Any] = {}
    # Prefer an explicit lab dir's .spa.yml, then walk from start.
    if env_lab:
        explicit_yml = _expand(env_lab, start) / SPA_YML_NAME
    ...
    if env_lab:
        spa_lab_dir = _expand(env_lab, start)
    elif spa_yml_path is not None:
        spa_lab_dir = spa_yml_path.parent.resolve()
    else:
        spa_lab_dir = spa_home

    if env_config:
        config_file = _expand(env_config, start)
    else:
        config_file = (spa_lab_dir / "config" / "splunk_config.yml").resolve()

    inventory_dir = (spa_lab_dir / "inventory").resolve()
    terraform_modules_dir = (spa_home / "terraform" / "aws").resolve()
    # Equal roots: keep writing into terraform/aws/ (in-repo labs do not move).
    terraform_state_dir = (spa_lab_dir / "terraform" / "aws").resolve()
    roots_differ = spa_home != spa_lab_dir
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Resolve SPA_HOME / SPA_LAB_DIR / SPLUNK_CONFIG_FILE (Distribution M1)"
    )
    parser.add_argument(
        "--json",
        action="store_true",
